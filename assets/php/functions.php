...
// ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//for checking duplicate mobile number
function isMobileNumberRegistered($phone)
{
    global $db;
...
function isRegistrationNumberRegistered($registration_number)
{
    global $db;
    $query = "SELECT count(*) as row FROM users WHERE registration_number='$registration_number'";
    $run = mysqli_query($db, $query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}

//for checking duplicate registration number
function isEnrollmentNumberRegistered($enrollment_number)
{
    global $db;
    $query = "SELECT count(*) as row FROM users WHERE enrollment_number='$enrollment_number'";
    $run = mysqli_query($db, $query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}

function checkRegistrationNumber($registration_number)
{
    global $db;
    // Check if the registration number exists in the database
    $query = "SELECT COUNT(*) AS row FROM students WHERE registration_number = '$registration_number'";
    $run = mysqli_query($db, $query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}
...
function isEmailRegisteredByOther($email)
{
    global $db;
    $user_id = $_SESSION['userdata']['id'];
    $query = "SELECT count(*) as row FROM users WHERE email='$email' && id!=$user_id";
    $run = mysqli_query($db, $query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}

function isMobileNumberRegisteredByOther($phone)
{
    global $db;
    $user_id = $_SESSION['userdata']['id'];
    $query = "SELECT count(*) as row FROM users WHERE phone='$phone' && id!=$user_id";
    $run = mysqli_query($db, $query);
    $return_data = mysqli_fetch_assoc($run);
    return $return_data['row'];
}
...
function validateSignupForm($form_data)
{
    ...
    if (isMobileNumberRegistered($form_data['phone'])) {
    ...
    if (isRegistrationNumberRegistered($form_data['registration_number'])) {
        $response['msg'] = "Registration Number is already registered";
        $response['status'] = false;
        $response['field'] = 'registration_number';
    }
    if (!checkRegistrationNumber($form_data['registration_number'])) {
        $response['msg'] = "Registration Number is Invalid";
        $response['status'] = false;
        $response['field'] = 'registration_number';
    }
    if (isEnrollmentNumberRegistered($form_data['enrollment_number'])) {
        $response['msg'] = "Enrollment Number is already registered";
        $response['status'] = false;
        $response['field'] = 'enrollment_number';
    }
    return $response;
}
...
{
    global $db;
    $query = "SELECT * FROM users WHERE username LIKE '%" . $keyword . "%' || (first_name LIKE '%" . $keyword . "%' || name LIKE '%" . $keyword . "%') LIMIT 5";
    $run = mysqli_query($db, $query);
    return mysqli_fetch_all($run, true);
}
...
{
    global $db;
    $query = "SELECT users.id as uid,posts.id,posts.user_id,posts.post_img,posts.post_text,posts.created_at,users.name,users.username,users.profile_pic FROM posts JOIN users ON users.id=posts.user_id ORDER BY id DESC";
    $run = mysqli_query($db, $query);
    return mysqli_fetch_all($run, true);
}
...
function createUser($data)
{
    ...
    $registration_number = mysqli_real_escape_string($db, $data['registration_number']);
    $enrollment_number = mysqli_real_escape_string($db, $data['enrollment_number']);
    $query = "INSERT INTO users(name,gender,email,phone,username,password,registration_number,enrollment_number) ";
    $query .= "VALUES ('$name',$gender,'$email',$phone,'$username','$password','$registration_number','$enrollment_number')";
    return mysqli_query($db, $query);
}
...
function validateUpdateForm($form_data, $image_data)
{
    ...
    if (isEmailRegisteredByOther($form_data['email'])) {
        $response['msg'] = $form_data['email'] . " is already registered";
        $response['status'] = false;
        $response['field'] = 'email';
    }
    if (isMobileNumberRegisteredByOther($form_data['phone'])) {
        $response['msg'] = $form_data['phone'] . " is already registered";
        $response['status'] = false;
        $response['field'] = 'phone';
    }
    if ($image_data['name']) {
        ...
        if ($size > 25) {
            $response['msg'] = "Upload image less then 25 mb";
            $response['status'] = false;
            $response['field'] = 'profile_pic';
        }
    }
...
function updateProfile($data, $imagedata)
{
    ...
    $query = "UPDATE users SET name='$name',username='$username',password='$password', email='$email',phone='$phone' $profile_pic WHERE id=" . $_SESSION['userdata']['id'];
    return mysqli_query($db, $query);
}
