...
if (isset($_SESSION['Auth']) && $user['ac_status'] == 1 && !$pagecount) {
    ...
    if (!$profile) {
        ...
        showPage('header', ['page_title' => $profile['name']]);
        showPage('navbar');
        showPage('profile');
    }

} elseif (isset($_GET['signup'])) {
    showPage('header', ['page_title' => 'StuSoc - SignUp']);
    showPage('signup');
} elseif (isset($_GET['login'])) {

    showPage('header', ['page_title' => 'StuSoc - Login']);
    showPage('login');
} elseif (isset($_GET['forgotpassword'])) {

    showPage('header', ['page_title' => 'StuSoc - Forgot Password']);
